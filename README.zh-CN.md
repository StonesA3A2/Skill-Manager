...
## 关于「中央仓库路径」（如果你感到困惑，请读这一节）

**设置 → 全局配置 → 中央仓库路径** 只是你自己电脑上的一个普通本地文件夹（默认：`~/.skills-manager`），应用在这里存放你的技能文件、SQLite 数据库和缓存。之所以叫「仓库」，只是因为它可以选择性地变成一个 Git 仓库以支持备份功能——**它不是、也和这个应用本身的源代码或它的 GitHub 仓库毫无关系。** 不会从 GitHub 下载或克隆任何东西到这里。更改这个路径只是把你本地的技能库挪到磁盘上的另一个位置。

这一行有三个各自独立的控件：

- **更改目录**（铅笔图标）——打开一个文本框。点击旁边的文件夹图标会弹出操作系统原生的文件夹选择对话框（带有常见的「选择文件夹」/确定按钮），也可以直接输入路径，然后点击对勾保存。更改路径会把现有的库移动到那里，需要重启应用才能完成（应用会提供一个重启按钮）。
- **恢复默认**（仅当设置了自定义路径后才会显示）——恢复为 `~/.skills-manager`。
- **查看当前文件夹**——只是在文件资源管理器中打开*当前*路径，方便你查看里面有什么。这是单向的「查看」操作，不是选择对话框——所以这里没有确定按钮，因为你不是在这里做选择。
