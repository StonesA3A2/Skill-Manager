...
### 发布概览
- 修复补丁：暗色模式下技能源码差异视图（diff view）文字不可读。

### 用户可见更新
- 差异视图（例如安装更新前查看技能源码变更）在暗色模式下现在可以正常阅读——此前新增/删除的行使用近乎全白的文字配浅色背景，实际上无法看清内容。

### 开发者与治理更新
- `DocumentDiffViewer.tsx` 的差异行配色使用固定的浅色背景（`#ffebe9`/`#dafbe1`，GitHub 风格，不随应用主题变化），却叠加了一个本应配合深色背景使用的 `dark:` 文字颜色覆盖——该覆盖在此场景下从未真正适用。移除 `dark:` 变体后，差异文字始终使用与其固定浅色背景相配的深色、可读配色。
## [0.5.1] - 2026-08-31
...
### 发布概览
- 完整的 MCP 服务器与 Claude Code 插件管理：添加、按代理部署/移除，插件还可查看其中包含的技能。此外还有若干设置页与安装页的体验修复。

### 用户可见更新
- **MCP 服务器管理** — 在新的 MCP 服务器页面添加服务器（名称、命令、参数、可选环境变量），一键按代理部署或移除。只会修改该服务器在代理自身配置文件中的那一条记录，其他任何已有条目（无论来自该代理还是其他工具）都不受影响，且每次写入前都会创建带时间戳的备份。目前支持：Claude Code、Cursor、Windsurf、Claude Desktop。
- **Claude Code 插件管理** — 在新的插件页面通过市场密钥、市场 Git URL 与插件 ID 添加插件，然后按代理启用或禁用。展开插件即可加载并列出其中包含的技能（从市场清单读取，而非猜测）。启用/未启用徽章显示该插件是否已在任意代理上启用。
- **技能库：独立技能 / 插件技能** — 技能库顶部新增切换开关，可在常规技能库与当前已启用插件所含技能的只读视图之间切换。
- **安装技能 → Git 安装项** — 从 Git 安装的技能现在按仓库分组（附加入日期），可按仓库折叠，「已安装」徽章只标记实际在使用中的技能（在某个预设中，或已部署到某个代理），而非每个已导入的技能。
- 全新安装后的默认语言现为英语，而非中文。
- 修复了侧边栏 Logo 显示错误图标的问题。
- 设置页：版本/更新面板在窄宽度下不再截断文字；「中央仓库路径」的长段说明文字已移至帮助指南，标题旁新增信息图标。
- 帮助指南已为上述两个新功能及两个增强功能补充说明条目。
- 以上所有内容现已支持全部 10 种语言，不再仅限英语/德语。

### 开发者与治理更新
- 新增 `core::mcp_adapters` 与 `core::plugin_adapters` 模块，共用新的 `core::json_config_io` 辅助模块（从 MCP 模块中提取），用于「读取 → 备份 → 写入」这一在他方也拥有的配置文件中精确修改单个 JSON 条目的通用模式。
- 新增数据库表：`mcp_servers`、`mcp_server_targets`、`plugin_marketplaces`、`plugins`、`plugin_targets`（迁移 v7→v9）。
- 新增 Tauri 命令（`commands::mcp`、`commands::plugins`）与 CLI 子命令（`mcp`、`plugins`），包括 `list_plugin_skills`——克隆插件市场仓库（复用现有 Git 缓存）并读取 `.claude-plugin/marketplace.json` 以解析插件实际的源子路径后再扫描。
- `vite.config.ts` 此前缺少对 `src-tauri/target` 的 `server.watch.ignored` 配置，导致 Vite 的文件监听与 Cargo 自身对构建产物的写入发生竞争，在 Windows 上可能以 `EBUSY` 使 `tauri dev` 崩溃，现已修复。
- 两个新适配器模块新增 15 个 Rust 单元测试（配置合并正确性、备份创建、适配器路径解析）；完整测试套件验证为 456/458（2 个既有失败与本次改动无关，是 Windows 符号链接权限相关的测试）。
