...
## 關於「中央倉庫路徑」（如果你感到困惑，請讀這一節）

**設定 → 全域設定 → 中央倉庫路徑** 只是你自己電腦上的一個普通本地資料夾（預設：`~/.skills-manager`），應用在這裡存放你的技能檔案、SQLite 資料庫和快取。之所以叫「倉庫」，只是因為它可以選擇性地變成一個 Git 倉庫以支援備份功能——**它不是、也和這個應用本身的原始碼或它的 GitHub 倉庫毫無關係。** 不會從 GitHub 下載或複製任何東西到這裡。變更這個路徑只是把你本地的技能庫搬到磁碟上的另一個位置。

這一行有三個各自獨立的控制項：

- **變更目錄**（鉛筆圖示）——打開一個文字方塊。點擊旁邊的資料夾圖示會彈出作業系統原生的資料夾選擇對話框（帶有常見的「選擇資料夾」/確定按鈕），也可以直接輸入路徑，然後點擊打勾儲存。變更路徑會把現有的庫移動到那裡，需要重新啟動應用才能完成（應用會提供一個重新啟動按鈕）。
- **恢復預設**（僅當設定了自訂路徑後才會顯示）——恢復為 `~/.skills-manager`。
- **檢視目前資料夾**——只是在檔案總管中打開*目前*路徑，方便你查看裡面有什麼。這是單向的「檢視」操作，不是選擇對話框——所以這裡沒有確定按鈕，因為你不是在這裡做選擇。
